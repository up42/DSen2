...
def fixture_superresolution_clip():
    ...
    return Superresolution("a.SAFE", "50.550671,26.15174,50.596161,26.19195", True, "/tmp/output")

# pylint: disable=redefined-outer-name
# @pytest.fixture(scope="session")
# def fixture_superresolution_clip():
#     return data10, data20, data60, [xmin, ymin, xmax, ymax], pr
...
def test_start(fixture_superresolution_clip, monkeypatch):
    ...
    data10, data20, data60, [xmin, ymin, xmax, ymax], pr = fixture_superresolution_clip.start()
    ...
    assert pr == {'driver': 'GTiff', 'dtype': 'uint16', 'nodata': None, 'width': 1584, 'height': 1762, 'count': 4,
                  'crs': CRS.from_epsg(32639), 'transform': Affine(10.0, 0.0, 454590.0,0.0, -10.0, 2898770.0),
                  'blockxsize': 128, 'blockysize': 128, 'tiled': True, 'interleave': 'pixel'}


def test_inference(fixture_superresolution_clip, monkeypatch):
    _location_ = os.path.realpath(os.path.join(os.getcwd(), os.path.dirname(__file__)))
    fixture_superresolution_clip.validated_10m_bands = ['B4', 'B3', 'B2', 'B8']
    fixture_superresolution_clip.validated_20m_bands = ['B5', 'B6', 'B7', 'B8A', 'B11', 'B12']
    fixture_superresolution_clip.validated_60m_bands = ['B1', 'B9']
    fixture_superresolution_clip.validated_descriptions_all = {'B4': 'B4 (665 nm)',
                                                               'B3': 'B3 (560 nm)',
                                                               'B2': 'B2 (490 nm)',
                                                               'B8': 'B8 (842 nm)',
                                                               'B5': 'B5 (705 nm)',
                                                               ...
                                                               'B1': 'B1 (443 nm)',
                                                               'B9': 'B9 (945 nm)'}
    fixture_superresolution_clip.data_name = "S2B_MSIL1C_20200708T070629_N0209_R106_T39RVJ_20200708T100303.SAFE"
    ...
    pr = {'driver': 'GTiff', 'dtype': 'uint16', 'nodata': None, 'width': 1584, 'height': 1762, 'count': 4,
          'crs': CRS.from_epsg(32639), 'transform': Affine(10.0, 0.0, 454590.0, 0.0, -10.0, 2898770.0),
          'blockxsize': 128, 'blockysize': 128, 'tiled': True, 'interleave': 'pixel'}

    fixture_superresolution_clip.inference(data10, data20, data60, coord, pr)
    result_path = os.path.join("/tmp/output", fixture_superresolution_clip.data_name.split(".")[0] + "_superresolution.tif")
